...
class REMIFullSongTransformerDataset(Dataset):
  ...
  def get_attr_classes(self, piece, st_bar):
    ...
    emote_cls  = pickle_load(os.path.join(self.data_dir, 'attr_cls/emotion', piece))[st_bar : st_bar + self.model_max_bars]
    ...
    emote_cls.extend([0 for _ in range(self.model_max_bars - len(emote_cls))])

    assert len(polyph_cls) == len(rfreq_cls) == len(emote_cls) == self.model_max_bars

    return polyph_cls, rfreq_cls, emote_cls
  ...
  def __getitem__(self, idx):
    ...
    if self.use_attr_cls:
      polyph_cls, rfreq_cls, emote_cls = self.get_attr_classes(os.path.basename(self.pieces[idx]), st_bar)
      polyph_cls_expanded = np.zeros((self.model_dec_seqlen,), dtype=int)
      rfreq_cls_expanded = np.zeros((self.model_dec_seqlen,), dtype=int)
      emote_cls_expanded  = np.zeros((self.model_dec_seqlen,), dtype=int)
      for i, (b_st, b_ed) in enumerate(zip(bar_pos[:-1], bar_pos[1:])):
        ...
        emote_cls_expanded[b_st:b_ed]  = emote_cls[i]
    else:
      polyph_cls, rfreq_cls, emote_cls = [0], [0], [0]
      polyph_cls_expanded, rfreq_cls_expanded, emote_cls_expanded = [0], [0], [0]
