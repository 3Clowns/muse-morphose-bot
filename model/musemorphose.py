...
class MuseMorphose(nn.Module):
  def __init__(self, enc_n_layer, enc_n_head, enc_d_model, enc_d_ff, 
    ...
    d_rfreq_emb=32, d_polyph_emb=32, d_emote_emb=32,
    n_rfreq_cls=8, n_polyph_cls=8, n_emote_cls=4,
    ...
    if use_attr_cls:
      self.decoder = VAETransformerDecoder(
        dec_n_layer, dec_n_head, dec_d_model, dec_d_ff, d_vae_latent + d_polyph_emb + d_rfreq_emb + d_emote_emb,
    ...
    if use_attr_cls:
      ...
      self.emote_attr_emb = TokenEmbedding(n_emote_cls,  d_emote_emb,  d_emote_emb)
    else:
      self.rfreq_attr_emb = None
      self.polyph_attr_emb = None
      self.emote_attr_emb = None
  ...
  def generate(self, inp, dec_seg_emb, rfreq_cls=None, polyph_cls=None, emote_cls=None, keep_last_only=True):
    token_emb = self.token_emb(inp)
    dec_inp = self.emb_dropout(token_emb) + self.pe(inp.size(0))

    if None not in (rfreq_cls, polyph_cls, emote_cls):
      dec_rfreq_emb = self.rfreq_attr_emb(rfreq_cls)
      dec_polyph_emb = self.polyph_attr_emb(polyph_cls)
      dec_emote_emb  = self.emote_attr_emb(emote_cls)
      if dec_seg_emb.dim() == 2:
        dec_seg_emb = dec_seg_emb.unsqueeze(1)
      dec_seg_emb_cat = torch.cat([dec_seg_emb, dec_rfreq_emb, dec_polyph_emb, dec_emote_emb], dim=-1)
    else:
  ...
  def forward(self, enc_inp, dec_inp, dec_inp_bar_pos, rfreq_cls=None, polyph_cls=None, emote_cls=None, padding_mask=None):
    ...
    if None not in (rfreq_cls, polyph_cls, emote_cls) and self.use_attr_cls:
      dec_rfreq_emb = self.rfreq_attr_emb(rfreq_cls)
      dec_polyph_emb = self.polyph_attr_emb(polyph_cls)
      dec_emote_emb  = self.emote_attr_emb(emote_cls)
      dec_seg_emb_cat = torch.cat([dec_seg_emb, dec_rfreq_emb, dec_polyph_emb, dec_emote_emb], dim=-1)
