...
def log_epoch(log_file, log_data, is_init=False):
  ...
  wandb.log({
    "epoch": log_data['ep'],
    "recons_loss": log_data['recons_loss'],
    "kl_loss": log_data['kldiv_loss'],
    "kl_raw": log_data['kldiv_raw'],
    "epoch_time": log_data['time'],
...
def train_model(epoch, model, dloader, dloader_val, optim, sched):
  ...
  for batch_idx, batch_samples in enumerate(dloader):
    ...
    batch_emote_cls  = batch_samples['emote_cls'].permute(1, 0).to(device)
    global trained_steps
    trained_steps += 1

    mu, logvar, dec_logits = model(
      batch_enc_inp, batch_dec_inp, 
      batch_inp_bar_pos, batch_rfreq_cls, batch_polyph_cls, batch_emote_cls,
    ...
    wandb.log({
      "train/recons_loss": recons_loss_ema,
      "train/kl_loss": kl_loss_ema,
      "train/kl_raw": kl_raw_ema,
      "train/beta": kl_beta,
      "train/lr": optim.param_groups[0]['lr'],
      "train/grad_norm": torch.nn.utils.clip_grad_norm_(model.parameters(), 0.5),
      "train/batch_time": time.time() - st
    }, step=trained_steps)
    ...
    if not trained_steps % val_interval:
      ...
      model.train()
      wandb.log({
        "val/recons_loss": np.mean(vallosses[0]),
        "val/kl_raw": np.mean(vallosses[1])
      }, step=trained_steps)
    ...
    if trained_steps % (log_interval * 10) == 0:
        for name, param in model.named_parameters():
            wandb.log({f"params/{name}": wandb.Histogram(param.data.cpu())}, step=trained_steps)
            if param.grad is not None:
                wandb.log({f"grads/{name}": wandb.Histogram(param.grad.cpu())}, step=trained_steps)
...
def validate(model, dloader, n_rounds=8, use_attr_cls=True):
  ...
  wandb.log({
      "val/mean_recons_loss": np.mean(loss_rec),
      "val/mean_kl_raw": np.mean(kl_loss_rec),
      "val/std_recons_loss": np.std(loss_rec),
      "val/max_recons_loss": np.max(loss_rec)
  })
...
if __name__ == "__main__":
  ...
  model = MuseMorphose(
    ...
    d_polyph_emb=mconf['d_polyph_emb'], d_rfreq_emb=mconf['d_rfreq_emb'], d_emote_emb=mconf['d_emote_emb'],
    cond_mode=mconf['cond_mode']
  ).to(device)
  if pretrained_params_path:
    state = torch.load(pretrained_params_path, map_location='cpu')
  ...
  wandb.init(
    project="MuseMorphose",
    config=config,
    dir=ckpt_dir,
    name=f"{config['experiment_name']}_{time.strftime('%Y%m%d-%H%M%S')}",
    save_code=True
  )
